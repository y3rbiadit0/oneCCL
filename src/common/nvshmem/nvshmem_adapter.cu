...
cudaStream_t get_stream(void* native_stream) {
    return reinterpret_cast<cudaStream_t>(native_stream);
}
...
extern "C" int oneccl_nvshmem_get_uid(void* uid, size_t size) {
    clear_error();
...
extern "C" int oneccl_nvshmem_cuda_get_device(int* device) {
    clear_error();
    if (device == nullptr) {
        std::snprintf(last_error, sizeof(last_error), "CUDA device output is null");
        return -1;
    }
    const cudaError_t status = cudaGetDevice(device);
    return (status == cudaSuccess) ? 0 : set_cuda_error("cudaGetDevice", status);
}

extern "C" int oneccl_nvshmem_cuda_set_device(int device) {
    clear_error();
    const cudaError_t status = cudaSetDevice(device);
    return (status == cudaSuccess) ? 0 : set_cuda_error("cudaSetDevice", status);
}

extern "C" int oneccl_nvshmem_is_initialized(void) {
    return nvshmemx_init_status() >= NVSHMEM_STATUS_IS_INITIALIZED;
}

extern "C" int oneccl_nvshmem_hostlib_init(int rank,
                                             int size,
                                             const void* uid,
                                             size_t uid_size) {
    clear_error();
    ...
    return (status == 0) ? 0 : set_nvshmem_error("nvshmemx_hostlib_init_attr", status);
}

extern "C" int oneccl_nvshmem_my_pe(void) {
    return nvshmem_my_pe();
}

extern "C" int oneccl_nvshmem_n_pes(void) {
    return nvshmem_n_pes();
}

extern "C" void* oneccl_nvshmem_malloc(size_t size) {
    clear_error();
...
extern "C" void oneccl_nvshmem_free(void* ptr) {
    if (ptr != nullptr) {
        nvshmem_free(ptr);
    }
}

extern "C" int oneccl_nvshmem_copy_async(void* destination,
                                           const void* source,
                                           size_t size,
                                           void* native_stream) {
    clear_error();
    ...
    if (destination == nullptr || source == nullptr || native_stream == nullptr) {
        std::snprintf(last_error, sizeof(last_error), "invalid asynchronous copy argument");
        return -1;
    }
    const cudaError_t status = cudaMemcpyAsync(
        destination, source, size, cudaMemcpyDefault, get_stream(native_stream));
    return (status == cudaSuccess) ? 0 : set_cuda_error("cudaMemcpyAsync", status);
}

extern "C" void oneccl_nvshmem_hostlib_finalize(void) {
    nvshmemx_hostlib_finalize();
}

extern "C" const char* oneccl_nvshmem_last_error(void) {
    return last_error;
}
